...
find_device_path_by_uuid() {
  local UUID=$1
  local DEVICE=$( blkid | grep "UUID=\"$UUID\"" | awk -F ':' '{print $1}')
  if [ -z "$DEVICE" ]; then
...
if [ -z "$MOUNT_OPTIONS" ]; then
   ...
   mount -o $MOUNT_OPTIONS $DISK $MOUNT_POINT
fi
...
if [ -z "$MOUNT_OPTIONS" ]; then
    echo "UUID=$UUID $MOUNT_POINT ext4 defaults 0 0" |  tee -a /etc/fstab
else
    echo "UUID=$UUID $MOUNT_POINT ext4 $MOUNT_OPTIONS 0 0" |  tee -a /etc/fstab
fi
