...
gcloud compute ssh $VM_USER@$INSTANCE_NAME --zone=$ZONE --command="bash <(curl -s https://raw.githubusercontent.com/uditgaurav/gcp-disk-mount/master/scripts/get-disk-uuid.sh) ${1} $DEVICE_NAME $MOUNT_POINT"
